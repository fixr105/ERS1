...
nodes.push(
  ...
  code(
    ...
    `const b=$input.first().json.body||$input.first().json;const projects=String(b.projectsIdentified||'');const count=projects.split(',').map(s=>s.trim()).filter(Boolean).length;const low=count<3;const prompt=\`You are a sharp interviewer at Seven Fincorp. Generate EXACTLY 10 questions tied to this employee's Stage 1 and Stage 2. Not generic HR.\\n\\nSTAGE 1:\\n\${b.stage1Summary||''}\\n\\nSTAGE 2 SUMMARY:\\n\${b.stage2Summary||''}\\n\\nProjects (\${count}): \${projects}\\nMeaningful-project threshold: \${low?'UNDER 3 — Q9 and Q10 must be the fixed reflection questions below':'3+ projects — Q9/Q10 must be project-specific'}\\n\\nQ1 Reasoning (why on a concrete artifact)\\nQ2-Q3 Gap Analysis (Stage 1 vs evidence)\\nQ4-Q6 Advice / how else to approach it\\nQ7-Q8 Project deep dive\\nQ9-Q10 \${low?'FIXED: (q9) What is the most important thing you learned this month and how will it change how you work? (q10) What does this month reveal about how you work at your best and where you still have room to grow?':'project-level learn / do differently'}\\n\\nReturn ONLY a JSON array of 10 objects {id,question,category} ids q1..q10. Categories: Reasoning|Gap Analysis|Decision Making|Advice|Project Deep Dive|Reflection\`;return [{json:{prompt,sessionId:b.sessionId,lowProjectCount:low}}];`,
    240,
    1400,
  ),
  aiAgent(
    ...
    480,
    1400,
    'You are a sharp interviewer at Seven Fincorp. Return ONLY a JSON array of 10 {id,question,category} objects. No markdown.',
  ),
